...
packet_run=Path('build/cp32-alloc-packet-trial1');rows+=json.loads((packet_run/'discovery-attached.json').read_text())['rows']
byname={r['symbol']:r for r in rows}
proof=json.loads(Path(args.baseline).read_text());roots={}
for r in proof['regions']:
 if r['unit']=='closure_vendor_graph_node_identity' and r['anchor'] in byname:
  roots[r['anchor']]=r['rva']
ctor=byname['??0ConnectionGraph@@QAE@XZ'];assert len(ctor['candidates'])==1
vt=ctor['candidates'][0]['inferred_targets']['??_7ConnectionGraph@@6B@']-p.base
import re
for row in rows:
 n=row['symbol']
 if re.match(r'^\?[^?@][^@]*@ConnectionGraph@@',n) or n.startswith(('??0ConnectionGraph@@','??1ConnectionGraph@@')):
  if n.startswith('?Update@ConnectionGraph@@'):roots[n]=u32(p.read(vt+3*4,4),0)-p.base
  elif n.startswith('?GetGraph@ConnectionGraph@@'):continue
  else:
   assert len(row['candidates'])==1,(n,row['candidates']);roots[n]=row['candidates'][0]['rva']
...
def read_initialized(rva,size):
 try:return p.read(rva,size)
  ...
  return bytes(size)
objects={u:COFF((packet_run if u=='closure_vendor_alloc_packet' else run)/(u+'.obj')) for u in {r['unit'] for r in rows}}
...
for unit,seed in out['seeds'].items():
 for name in list(seed):
  if name in existing and existing[name]['unit']!='closure_vendor_graph_node_identity':
...
out['explicit_owner_transfer']='closure_vendor_graph_node_identity -> closure_vendor_connection_graph; remove former TU from active build, preserve proofs/source.'
out['unselected_method']='Unreferenced homologues are excluded; all Nat roots linked to actual calls, data, allocation, and VFT.'
out['source_files']=[dict(path=str(f),sha256=hashlib.sha256(f.read_bytes()).hexdigest()) for f in [Path('client/saco/closure_vendor_connection_graph.cpp'),Path('vendor/upstream/raknet/ConnectionGraph.cpp'),Path('vendor/upstream/raknet/ConnectionGraph.h')]]
out['objects']={unit:hashlib.sha256(((packet_run if unit=='closure_vendor_alloc_packet' else run)/(unit+'.obj')).read_bytes()).hexdigest() for unit in objects}
from draft_actor_contract import weak_aliases
out['actual_weak_aliases']={unit:weak_aliases(c) for unit,c in objects.items()}
Path('build/agent-independent/connection-graph-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/connection-graph-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
