...
rows=json.loads(Path('build/agent-independent/disconnect-requests-trial1-discovery/discovery-attached.json').read_text())['rows']
...
proposal=json.loads(Path('build/agent-independent/rakpeer-disconnect-requests-proposal.json').read_text())
...
out['explicit_owner_transfer']='None; disconnect/request methods reuse existing owners.'
out['unselected_method']='Only real disconnect/request roots and original relocation closure selected; callback implementations are never credited.'
...
Path('build/agent-independent/rakpeer-disconnect-requests-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/rakpeer-disconnect-requests-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
