...
roots={}
ctor=next(r for r in rows if r['symbol'].startswith('??0NatPunchthrough@@'))
assert len(ctor['candidates'])==1
vt=ctor['candidates'][0]['inferred_targets']['??_7NatPunchthrough@@6B@']-p.base
for row in rows:
 n=row['symbol']
 if ('@NatPunchthrough@@' in n and 'ConnectionRequest' not in n) or n.startswith(('??0NatPunchthrough@@','??1NatPunchthrough@@')):
  if n.startswith(('?OnDisconnect@NatPunchthrough@@',)):
   roots[n]=u32(p.read(vt+5*4,4),0)-p.base
  elif n.startswith('?FacilitateConnections@NatPunchthrough@@'):
   continue
  elif not n.startswith(('??_G','??_E')):
...
out['source_files']=[dict(path=str(f),sha256=hashlib.sha256(f.read_bytes()).hexdigest()) for f in [Path('client/saco/closure_vendor_nat_punchthrough.cpp'),Path('vendor/upstream/raknet/NatPunchthrough.cpp'),Path('vendor/upstream/raknet/NatPunchthrough.h')]]
...
Path('build/agent-independent/nat-punchthrough-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/nat-punchthrough-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
