...
for row in rows:
 n=row['symbol']
 if n in ['??1FullyConnectedMesh@@UAE@XZ','?Initialize@FullyConnectedMesh@@QAEXPBD@Z','?OnReceive@FullyConnectedMesh@@UAE?AW4PluginReceiveResult@@PAVRakPeerInterface@@PAUPacket@@@Z']:
  assert len(row['candidates'])==1;roots[n]=row['candidates'][0]['rva']
roots['??0FullyConnectedMesh@@QAE@XZ']=0x2e600
vt=0xe6dbc
assert u32(p.read(vt+16,4),0)==p.base+roots['?OnReceive@FullyConnectedMesh@@UAE?AW4PluginReceiveResult@@PAVRakPeerInterface@@PAUPacket@@@Z']
for n,slot in [('?Update@FullyConnectedMesh@@UAEXPAVRakPeerInterface@@@Z',3),('?OnDisconnect@FullyConnectedMesh@@UAEXPAVRakPeerInterface@@@Z',5),('??_GFullyConnectedMesh@@UAEPAXI@Z',10)]:roots[n]=u32(p.read(vt+slot*4,4),0)-p.base
...
out['source_files']=[dict(path=str(f),sha256=hashlib.sha256(f.read_bytes()).hexdigest()) for f in [Path('client/saco/closure_vendor_fully_connected_mesh.cpp'),Path('vendor/upstream/raknet/FullyConnectedMesh.cpp'),Path('vendor/upstream/raknet/FullyConnectedMesh.h')]]
...
out['identity_details']={'ctor':'2E600 whole body references E6DBC, whose OnReceive slot points to unique445-byte2E630. Other candidate2EC30 references DatabaseClient VFT and is rejected.','layout':'FullyConnectedMesh8 = primary vptr4 + actual password pointer4. Ctor zeroes pointer; dtor and Initialize free actual array.','virtual_boundaries':'OnReceive invokes actual RakPeer GetIndexFromPlayerID/+5C, PlayerIDToDottedIP and Connect with two actual PlayerID reads. No concrete transport implementation credit.','vft':'Whole11-slot44-byte E6DBC immediately precedes DatabaseClient E6DE8; weak deleting slot10 maps actual G2E610.'}
Path('build/mesh-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/mesh-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
