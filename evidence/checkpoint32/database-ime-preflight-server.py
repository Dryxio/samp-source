...
run='cp32-database-server-trial1';unit='closure_vendor_database_server'
...
roots={}
for r in rows:
 if '@LightweightDatabaseServer@@' not in r['symbol'] or '?$' in r['symbol']:continue
 candidates=[c for c in r['candidates'] if 0x2f2f0<=c['rva']<0x31200]
 if len(candidates)==1:roots[r['symbol']]=candidates[0]['rva']
assert roots['?DeserializeClientHeader@LightweightDatabaseServer@@IAEPAUDatabaseTable@1@PAVBitStream@RakNet@@PAVRakPeerInterface@@PAUPacket@@H@Z']==0x30060
# True source identities: strcmp table-name comparator2F2F0, DatabaseTable
# embedded Table at9C, password32 layout at3/23/43, query/update packet IDs,
# all principal methods adjacent2Fxxx..30xxx; all helpers follow real callsites.
...
old=json.load(open('config/checkpoint32/listbox-roster-contract.json'));owners={r['anchor']:p.base+r['rva']+r.get('anchor_offset',0) for r in old['regions']}
...
Path('build/database-server-reviewed-manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
