...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30e-two-contract.json'));run=Path('build/cp32-trial30f-sixth1');objs={'closure_vehicle_trailer_lookup':COFF(run/'closure_vehicle_trailer_lookup.obj')}
...
plan=[('closure_vehicle_trailer_lookup','?GetTrailer@R5VehicleTrailerLookupView@@QAEPAVCVehicle@@XZ',0xb7cf0,120)]
...
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},typed_layout_note='025 GetTrailer adapted R5 subtype exclusions2/4/7/6, native trailer4c4, invalidFFFF. Actual active FindIDFromGtaPtr/GetSlotState/GetAt and pNetGame targets. No storage/CRT.')
Path('build/trial30f/trailer-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30f/trailer-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique')
