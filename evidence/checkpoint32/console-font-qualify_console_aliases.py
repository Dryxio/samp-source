...
run=Path('build')/args.run;p=PE(Path('private/samp.dll'));q=PE(run/'closure.dll');maps=map_symbols(run/'closure.map');m=json.loads(Path('build/agent-independent/console-reviewed-manifest.json').read_text())
def addr(n):
...
def compare_section(si,n):
 ...
 return proof
out=[];object_hashes={}
for unit in ['eval_vendor_command_parser_interface','eval_vendor_log_command_parser','eval_vendor_packet_console_logger']:
...
assert len(out)==3
result=dict(status='PASS_THREE_TRUE_CONSOLE_DELETING_ALIASES',run=str(run),object_sha256=object_hashes,dll_sha256=hashlib.sha256(q.data).hexdigest(),map_sha256=hashlib.sha256((run/'closure.map').read_bytes()).hexdigest(),aliases=out,extra_code_credit=0)
Path('build/agent-independent/console-aliases-qualified.json').write_text(json.dumps(result,indent=2)+'\n');print('PASS',len(out),'complete code and VFT both images')
