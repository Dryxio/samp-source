...
rows=[]
for cls,unit,rva,table in [('PacketLogger','closure_vendor_packet_logger',0x33c80,0xe7250),('PacketFileLogger','closure_vendor_packet_file_logger',0x33a90,0xe71f8)]:
...
f=Path('build/agent-independent')/('logger-deleting-aliases-'+Path(args.run).name+'.json');f.write_text(json.dumps(a,indent=2)+'\n');print(f);print('PASS two storage105 weak aliases, canonical MAP addresses and selected full VFT fixups')
