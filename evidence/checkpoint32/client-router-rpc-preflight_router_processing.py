...
rows=json.loads(Path('build/agent-independent/router-processing-trial1-discovery/discovery-attached.json').read_text())['rows']
...
proposal=json.loads(Path('build/agent-independent/router-processing-proposal.json').read_text())
...
roots.update({'?GetWriteOffset@BitStream@RakNet@@QBEHXZ': 128800, '??$Write@_N@BitStream@RakNet@@QAEX_N@Z': 11600, '??$Write@UPlayerID@@@BitStream@RakNet@@QAEXUPlayerID@@@Z': 137216, '??1?$List@UPlayerID@@@DataStructures@@QAE@XZ': 134160, '??0?$List@UPlayerID@@@DataStructures@@QAE@XZ': 134144})
...
out['explicit_owner_transfer']='None; Router processing and SystemAddressList methods; existing providers retained.'
out['unselected_method']='Only real Router processing / SystemAddressList roots and original relocation closure selected; callback implementations are never credited.'
...
Path('build/agent-independent/router-processing-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/router-processing-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
