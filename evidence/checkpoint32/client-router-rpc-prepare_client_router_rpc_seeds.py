...
seeds=json.load(open('config/checkpoint32/disconnect-selection-seeds.json'));adds={}
for file in ['build/agent-independent/rakclient-messages-seeds.json','build/agent-independent/router-processing-seeds.json']:
 for u,rows in json.load(open(file)).items():adds.setdefault(u,{}).update(rows)
for r in json.load(open('build/root-rpc-control-seeds.json')):adds.setdefault(r['unit'],{})[r['symbol']]=r['rva']
...
for u,rows in adds.items():
 o=COFF(Path('build/cp32-client-router-rpc-ob1-v2')/(u+'.obj'))
...
exports.write_text(s+''.join(lines));Path('config/checkpoint32/client-router-rpc-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('Added exports',len(lines))
