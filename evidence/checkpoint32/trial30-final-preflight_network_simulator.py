...
rows=[r for r in json.loads(Path('build/agent-independent/socket-bootstrap-trial2-discovery/discovery-attached.json').read_text())['rows'] if r['unit']=='eval_vendor_network_simulator']
...
proposal=json.loads(Path('build/agent-independent/network-simulator-proposal.json').read_text())
...
Path('build/agent-independent/network-simulator-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/network-simulator-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
