...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30e-two-contract.json'));run=Path('build/cp32-trial30f-controls2');objs={'closure_vehicle_control_queries':COFF(run/'closure_vehicle_control_queries.obj')}
...
plan=[('closure_vehicle_control_queries', '?SetInvulnerable@R5VehicleControlQueriesView@@QAEXH@Z', 749968, 154), ('closure_vehicle_control_queries', '?SetHornState@R5VehicleControlQueriesView@@QAEXE@Z', 750800, 112), ('closure_vehicle_control_queries', '?SetWindow@R5VehicleControlQueriesView@@QAEXE@Z', 753904, 77), ('closure_vehicle_control_queries', '?IsValid@R5VehicleControlQueriesView@@QAEHXZ', 753984, 16), ('closure_vehicle_control_queries', '?GetTrainSpeed@R5VehicleControlQueriesView@@QAEMXZ', 751840, 30), ('closure_vehicle_control_queries', '?SetTrainSpeed@R5VehicleControlQueriesView@@QAEXM@Z', 751872, 54)]
...
covered=accepted_code('evidence/checkpoint32/trial30e-two-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)>0
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},typed_layout_note='Partial vehicle view gtaId44, vehicle4c, invulnerable58; actual native pool/subtype and scripting provider. Complete command descriptors and floats. No new storage CRT.')
Path('build/trial30f/controls-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30f/controls-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique')
