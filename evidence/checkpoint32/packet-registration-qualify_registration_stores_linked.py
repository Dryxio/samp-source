...
D=Path('build/cp32-packet-registration-linked1');p=PE(D/'closure.dll');maps={}
...
plan=[('?bQuitGame@@3HA','closure_store_object_quit',4,0),('?dwStartQuitTick@@3KA','closure_store_quit_start_tick',4,0),('?pDeathWindow@@3PAVCDeathWindow@@A','closure_store_death_window',4,0),('?iGameDebugType@@3HA','closure_store_game_debug_type',4,0),('?dwDebugEntity1@@3KA','closure_store_game_debug_entity1',4,0),('?dwDebugEntity2@@3KA','closure_store_game_debug_entity2',4,0),('?r5RpcChat@@3HB','closure_store_rpc_chat',4,101)];owners={n:[] for n,u,z,b in plan}
for f in D.glob('*.obj'):
 o=COFF(f)
 for n in owners:
...
result=dict(status='PASS_SOLE_REAL_STORES_AND_LINKED_INITIALIZATION',run=D.name,owners=rows);Path('build/agent-textdraw/registration-stores-linked-review.json').write_text(json.dumps(result,indent=2)+'\n');print(result['status'])
