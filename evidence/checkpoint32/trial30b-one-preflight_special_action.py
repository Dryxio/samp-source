...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30-final-contract.json'));run=Path('build/cp32-trial30b-special-action');objs={u:COFF(run/(u+'.obj')) for u in ['closure_remote_special_action_query']}
...
plan=[('closure_remote_special_action_query','?GetVehicleEntryTask@R5PedEntryTaskView@@QAEHXZ',0xac5d0,98),('closure_remote_special_action_query','?GetSpecialAction@R5RemoteSpecialActionView@@QAEHXZ',0x16330,64)]
providers['?IsExitingVehicle@R5PedExitTaskView@@QAEHXZ']=p.base+0xac640
...
covered=accepted_code('evidence/checkpoint32/trial30-final-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==162
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=0,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},identity_callers=[0x75330],typed_layout_note='Pointer-only views nativePED2A4/task10, remoteaction1C5/ped1DD. IsExitingVehicle70 dependency is onfoot manifest samebatch; no native allocations.')
Path('build/agent-client-b/special-action-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/agent-client-b/special-action-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
