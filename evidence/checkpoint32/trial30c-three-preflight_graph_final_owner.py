...
p=PE(Path('private/samp.dll'));run=Path('build/trial30c/network-agent/graph-final-owner-discovery')
rows=[r for r in json.loads((run/'discovery-attached.json').read_text())['rows'] if r['unit']=='closure_vendor_graph_algorithms']
byname={r['symbol']:r for r in rows}
roots={n:rv for n in byname for key,rv in [('GenerateDisjktraMatrix',329184),('GetShortestPath',330368),('GetSpanningTree',331408)] if key in n}
...
out['source_files']=[dict(path=str(f),sha256=hashlib.sha256(f.read_bytes()).hexdigest()) for f in [Path('build/cp32-trial30c-final-probe/client/saco/closure_vendor_graph_algorithms.cpp')]]
Path('build/trial30c/network-agent/graph-final-owner-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/trial30c/network-agent/graph-final-owner-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
