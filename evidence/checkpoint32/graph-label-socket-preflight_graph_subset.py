...
p=PE(Path('private/samp.dll'));run=Path('build/cp32-graph-algorithms-trial2')
...
out['source_files']=[dict(path=str(f),sha256=hashlib.sha256(f.read_bytes()).hexdigest()) for f in [Path('build/agent-independent/eval_vendor_graph_algorithms2_external_providers.cpp'),Path('build/agent-independent/eval_vendor_graph_node_identity2_providers.cpp'),Path('build/agent-independent/eval_vendor_graph_playerid_comparisons.cpp')]]
...
Path('build/agent-independent/graph-subset-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/graph-subset-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
