...
rows=json.loads(Path('build/agent-network-b/network1-discovery.json').read_text())['rows']
...
proposal=json.loads(Path('build/agent-network-b/network1-proposal.json').read_text())
...
out['explicit_owner_transfer']='None; RakPeer plugin and original queue lifetimes methods reuse existing providers.'
out['unselected_method']='Only real RakPeer plugin and original queue lifetimes roots and original relocation closure selected; callback implementations are never credited.'
...
Path('build/agent-network-b/network1-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-network-b/network1-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
