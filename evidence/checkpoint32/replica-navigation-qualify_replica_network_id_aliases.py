...
run=Path('build')/args.run;unit='closure_vendor_network_id_generator';o=COFF(run/(unit+'.obj'));p=PE(Path('private/samp.dll'));q=PE(run/'closure.dll');maps=map_symbols(run/'closure.map');m=json.loads(Path('build/agent-independent/replica-network-id-reviewed-manifest.json').read_text())
regions={r['section']:r for r in m['regions'] if r['unit']==unit}
def addr(n):
...
for e,g in weak_aliases(o).items():
 ...
 vs=next(s for s in o.names[v] if s['section']>0);sec=o.sections[vs['section']-1];fx=relocs(o,vs['section']);assert any(f['offset']==0 and f['kind']==6 and f['symbol']['name']==e for f in fx)
 assert sec['size']==(28 if g.startswith('??_GNetworkIDGenerator') else 4)
...
assert len(out)==3
result=dict(status='PASS_THREE_TRUE_NETWORK_ID_DELETING_ALIASES',run=str(run),new_owner=unit,object_sha256=hashlib.sha256(o.data).hexdigest(),dll_sha256=hashlib.sha256(q.data).hexdigest(),map_sha256=hashlib.sha256((run/'closure.map').read_bytes()).hexdigest(),aliases=out,extra_code_credit=0)
Path('build/agent-independent/replica-network-id-aliases-qualified.json').write_text(json.dumps(result,indent=2)+'\n');print('PASS',len(out),'complete code and VFT both images')
