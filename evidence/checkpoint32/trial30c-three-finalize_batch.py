...
files=set(spec['manifests']+spec['seed_files']+list(spec['linked_reviews'].values())+spec.get('evidence_files',[])+[sys.argv[1],'build/prepare_trial30b_batch.py','build/trial30c/review_batch.py','build/qualify_trial30b_link.py','build/trial30c/finalize_batch.py'])
for file in files:
...
p=Path('evidence/checkpoint32/PROGRESS.md');p.write_text(p.read_text()+f"\n\n{prefix}: +{c['new_unique_code_bytes']} unique accepted R5 code bytes, total {c['union_code_bytes']}/930756 ({c['text_percent']:.6f}%). {spec['review']['scope']} Complete source/linked code,data,EH, actual COFF/PE targets, no unimplemented provider credit; required negative controls and Actor ABI PASS. Trial C continues until 2026-09-08T13:25:51Z; no push/deployment.\n")
p=Path('build/trial30c/state.json');s=json.loads(p.read_text());s.setdefault('batches',[]).append(dict(prefix=prefix,gain=c['new_unique_code_bytes'],coverage=c['union_code_bytes'],utc=datetime.now(timezone.utc).isoformat(),prepared_before_trial=spec.get('prepared_before_trial',0)));s['current_accepted']=c['union_code_bytes'];p.write_text(json.dumps(s,indent=2)+'\n');print(c)
