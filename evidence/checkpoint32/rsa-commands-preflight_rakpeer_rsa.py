...
# Actual RSA methods tied to RakPeer source callers. Tiny constructor/destructor
# identities also require their sole reset target 3B0D0, not a body homologue.
anchors = {
 '?getPublicKey@?$RSACrypt@$$BY07I@big@@QAEXAAIAAY07I@Z':0x3B0B0,
 '?reset@?$RSACrypt@$$BY07I@big@@QAEXXZ':0x3B0D0,
 '?setPublicKey@?$RSACrypt@$$BY07I@big@@QAEXIAAY07I@Z':0x40B90,
 '?encrypt@?$RSACrypt@$$BY07I@big@@QAEXAAY07I0@Z':0x40BF0,
 '?decrypt@?$RSACrypt@$$BY07I@big@@QAEXAAY07I0@Z':0x40C60,
 '??$setPrivateKey@$$BY03I@?$RSACrypt@$$BY07I@big@@QAEXAAY03I0@Z':0x40CC0,
 '?generateKeys@?$RSACrypt@$$BY07I@big@@QAEXXZ':0x42740,
 '??0?$RSACrypt@$$BY07I@big@@QAE@XZ':0x3CC60,
 '??1?$RSACrypt@$$BY07I@big@@QAE@XZ':0x3CC70,
}
for name,rva in anchors.items():
 if name in byname:
  assert any(c['rva']==rva for c in byname[name]['candidates']),(name,hex(rva),'root unmatched')
  roots[name]=rva
...
out['explicit_owner_transfer']='None; new RSA algorithm instantiation owner, no complete RakPeer linkage.'
out['unselected_method']='Unused emitted specializations without actual original identity are excluded; RSA has no VFT or dynamic allocation.'
...
Path('build/agent-independent/rakpeer-rsa-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/rakpeer-rsa-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
