...
p=PE(Path('private/samp.dll'));run=Path('build/cp32-graph-label-socket-ob1-v2')
rows=json.loads((run/'discovery.json').read_text())['rows']
oldrun=Path('build/cp32-graph-algorithms-trial2')
oldrows=json.loads((oldrun/'discovery.json').read_text())['rows']
for u in ['closure_vendor_graph_node_identity','closure_vendor_graph_playerid_comparisons']:
 assert (run/(u+'.obj')).read_bytes()==(oldrun/(u+'.obj')).read_bytes(),u
rows=[r for r in rows if r['unit']=='closure_vendor_graph_algorithms']+[r for r in oldrows if r['unit']!='closure_vendor_graph_algorithms']

byname={r['symbol']:r for r in rows}
roots={f['symbol']:f['rva'] for f in json.loads(Path('build/agent-independent/graph-trial2-closure-diagnosis.json').read_text())['closed']}
 ...
 while changed:
  ...
  for n,va in list(ids.items()):
   for s in c.names.get(n,[]):
    if s['section']>0 and c.sections[s['section']-1]['name'] in ['.text$x','.xdata$x','.rdata','.data'] and s['section'] not in sections[unit]:
     sections[unit][s['section']]=va-p.base-s['value'];changed=True
  for si,rva in list(sections[unit].items()):
   sec=c.sections[si-1]
   if sec['name'] not in ['.text$x','.xdata$x','.rdata','.data']:continue
   ref=p.read(rva,sec['size'])
...
for unit,c in objects.items():
 for si,rva in sections[unit].items():
  sec=c.sections[si-1];raw=bytearray(sec['bytes']);ref=p.read(rva,len(raw));bs=[]
  for j in range(sec['reloc_count']):
...
proof_path='evidence/checkpoint32/plate-request-acceptance.json'
...
out['excluded_unmatched']=['GetShortestPath1026','GenerateDisjktraMatrix1170','GetSpanningTree1059 transitive dependency']
out['vector_CRT_qualification']='build/agent-independent/vector-crt-qualified-providers.json'
out['source_files']=[dict(path=str(f),sha256=hashlib.sha256(f.read_bytes()).hexdigest()) for f in [Path('build/agent-independent/eval_vendor_graph_algorithms3_explicit_comparison.cpp'),Path('build/agent-independent/eval_vendor_graph_node_identity2_providers.cpp'),Path('build/agent-independent/eval_vendor_graph_playerid_comparisons.cpp')]]
 ...
 for name,row in selected.items():
  for sy in c.names.get(name,[]):
   if sy['section']>0 and sy['type']==32 and sy['value']==0 and sy['section'] in sections[u] and c.sections[sy['section']-1]['name']=='.text':
    assert sections[u][sy['section']]==row['rva']
    if name not in out['seeds'][u] and name not in out['reuse_existing_owners']:
...
assert not any('defaultOrderedListComparison' in k for a in out['actual_weak_aliases'].values() for k in a)
out['local_label_scope']='Compiler $ labels are qualified per object during checking; binding records retain actual COFF spelling.'
out['all_emitted_copies_reviewed']=True
out['data_scope']='All 804 bytes of complete EH FuncInfo/unwind map sections verified; no global/table data dependencies.'
out['identity_scope']='build/agent-independent/graph-algorithms-proposal.json'
out['class_allocation_EH_scope']='build/agent-independent/graph-expected-eh-scope.json'
Path('build/agent-independent/graph-subset-v2-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/graph-subset-v2-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
