...
p=Path('build/trial30c/network-agent');m=json.load(open(p/'graph-final-owner-reviewed-manifest.json'));c=json.load(open('config/checkpoint32/trial30b-two-contract.json'));idx=json.load(open('build/trial30c/symbol-index.json'))['symbols'];o=COFF(p/'graph-final-owner-discovery/closure_vendor_graph_algorithms.obj');pe=PE('private/samp.dll');d=Cs(CS_ARCH_X86,CS_MODE_32)
...
report=dict(status='PASS_SCOPE_AND_OWNER_AUDIT_NOT_ACCEPTANCE',source_proposal='eval_vendor_graph_size_context_trial.cpp',real_owner='closure_vendor_graph_algorithms',new_unique=m['new_unique_code'],old_owner_regions=checks,throw_declaration_evidence=no_throw,external_target_identity=ext,weak_aliases=weak_aliases(o),layout='Real Node7,WeightedGraph52,GraphAdjacency24,GraphTree19,GraphHeap12 unchanged static asserts; no storage/global/CRT additions.',identity_source='Old source graph-algorithms-proposal matched three high Router algorithms, full call graph remains; all three graph algorithms3255 including215EH selected now. Each used real helper is already accepted and agrees originaltarget.',source025_direct_bytes=0,prepared_before_trial=False,adaptation='Fresh accurate throw() declarations recover exception knowledge; explicit real Queue::Size and outerMap::Size instantiations recover inlining and callee register knowledge lost under extern-template suppression. Extra physically emittedSizecopies fully matched with no duplicatecredit.',excluded='None of the three high algorithms remain excluded. Existing Dijkstra new gain must be deduced by root from latest accepted coverage.')
(p/'graph-final-actual-owner-scope-audit.json').write_text(json.dumps(report,indent=2)+'\n');print('PASS',len(checks),'old regions',len(no_throw),'nonthrowing providers',m['new_unique_code'],'new proposed')
