...
old=json.load(open('config/checkpoint32/policy-ui-contract.json'));new=json.load(open('build/actor-contract-draft.json'));prior={(r['unit'],r['rva'],r['size'],r['kind']):r for r in old['regions']}
run=Path('build/cp32-remote-state-linked1');pe=PE('private/samp.dll');objs={p.stem:COFF(p) for p in run.glob('*.obj')}
reviews=[json.load(open('build/agent-independent/rakpeer-remote-state-reviewed-manifest.json'))]
...
for r in new['regions']:
 ...
 r['implementation']='Normal C++ whole RakPeer remote state/RPCMap/reliability setters, all original and linked targets'
...
new['review']=dict(old['review']);new['review'].update(scope='RakPeer remote state and actual RPCMap/reliability setters; expected2816 unique.',adaptations='Original vendor definitions and full types, real RPCNode6 allocation. Whole statistics static POD296. No partial RakPeer or RemoteSystem allocation.',reference_credit='Direct025 new0, cumulative22268; vendor already in base.',verification='Cached incremental build; complete source code/data/EH and all COFF/PE targets, inherited ten controls and Actor ABI required.',excluded_candidates='Three old pending unchanged; no CRT/data/duplicate or unimplemented concrete virtual provider credit.')
for name,file in [('itoa_qualification','build/remote-state-itoa-qualified.json'),('linker_alias_provider_evidence','build/cp32-remote-state-linked1-aliases.json'),('server_bridge_final','build/agent-independent/server-bridge-target-alias-qualification-cp32-remote-state-linked1.json'),('logger_deleting_aliases','build/agent-independent/logger-deleting-aliases-cp32-remote-state-linked1.json'),('network_tree_deleting_aliases','build/agent-independent/replica-network-id-aliases-qualified.json'),('sha1_deleting_alias_qualification','build/remote-state-sha1-deleting-alias-qualified.json'),('mesh_deleting_alias_qualification','build/agent-independent/mesh-deleting-aliases-cp32-remote-state-linked1.json'),('chat_navigation_alias','build/agent-textdraw/chat-navigation-linked-alias-review.json')]:new['review'][name]=json.load(open(file))
new['review']['remote_state_scope']=json.load(open('build/agent-independent/rakpeer-remote-state-scope-reviewed.json'))
...
p=Path('config/checkpoint32/remote-state-contract.json');p.write_text(json.dumps(new,indent=2)+'\n')
