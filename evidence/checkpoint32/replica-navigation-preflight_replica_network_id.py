...
rows=[r for r in json.loads((run/'discovery-attached.json').read_text())['rows'] if r['unit'] in ['closure_vendor_replica_manager','closure_vendor_network_id_generator']]
rows+=json.loads(Path('build/agent-independent/replica-initializer-discovery.json').read_text())['rows']
byname={r['symbol']:r for r in rows}
proof=json.loads(Path(args.baseline).read_text());roots={}
for r in proof['regions']:
 if r['unit']=='closure_vendor_network_id_tree' and r['anchor'] in byname:roots[r['anchor']]=r['rva']
vtm=byname['??0ReplicaManager@@QAE@XZ']['candidates'][0]['inferred_targets']['??_7ReplicaManager@@6B@']-p.base
vtg=byname['??0NetworkIDGenerator@@QAE@XZ']['candidates'][0]['inferred_targets']['??_7NetworkIDGenerator@@6B@']-p.base
import re
for row in rows:
 n=row['symbol']
 if re.match(r'^\?[^?@][^@]*@(ReplicaManager|NetworkIDGenerator)@@',n) or n.startswith(('??0ReplicaManager@@','??1ReplicaManager@@','??0NetworkIDGenerator@@','??1NetworkIDGenerator@@')):
  if n.startswith(('?CommandStructComp','?RegisteredReplicaComp','?RemoteObjectComp','?GetReplicaCount','?GetStaticNetworkID')):continue
  if n.startswith('?OnAttach@ReplicaManager'):rv=u32(p.read(vtm,4),0)-p.base
  elif n.startswith('?OnDisconnect@ReplicaManager'):rv=u32(p.read(vtm+20,4),0)-p.base
  elif n.startswith('?OnCloseConnection@ReplicaManager'):rv=u32(p.read(vtm+24,4),0)-p.base
  elif n.startswith('?SetParent@NetworkIDGenerator'):rv=u32(p.read(vtg+12,4),0)-p.base
  elif n.startswith('?GetParent@NetworkIDGenerator'):rv=u32(p.read(vtg+16,4),0)-p.base
  elif n.startswith('?RequiresSetParent@NetworkIDGenerator'):rv=u32(p.read(vtg+24,4),0)-p.base
  else:
   assert len(row['candidates'])==1,(n,row['candidates']);rv=row['candidates'][0]['rva']
  roots[n]=rv
roots['_$E5']=934512
roots['_$E8']=934544
roots['??_GNetworkIDGenerator@@UAEPAXI@Z']=u32(p.read(vtg,4),0)-p.base
...
crt_unit='closure_vendor_network_id_generator';crt_c=objects[crt_unit]
crt_si=next(i+1 for i,v in enumerate(crt_c.sections) if v['name']=='.CRT$XCU')
crt_pattern=struct.pack('<II',p.base+934512,p.base+934544)
crt_hits=[]
for sec in p.sections:
 if sec['name']=='.data':
  data=p.read(sec['rva'],sec['size']);off=data.find(crt_pattern)
  while off>=0:
   rv=sec['rva']+off
   if rv%4==0 and rv in p.relocations and rv+4 in p.relocations:crt_hits.append(rv)
   off=data.find(crt_pattern,off+1)
assert len(crt_hits)==1,crt_hits
sections[crt_unit][crt_si]=crt_hits[0]
...
out['explicit_owner_transfer']='closure_vendor_network_id_tree -> closure_vendor_network_id_generator; remove former TU from active build, preserve proofs/source.'
out['unselected_method']='Unreferenced homologues are excluded; all Nat roots linked to actual calls, data, allocation, and VFT.'
out['source_files']=[dict(path=str(f),sha256=hashlib.sha256(f.read_bytes()).hexdigest()) for f in [Path('client/saco/closure_vendor_replica_manager.cpp'),Path('vendor/upstream/raknet/ReplicaManager.cpp'),Path('vendor/upstream/raknet/ReplicaManager.h'),Path('client/saco/closure_vendor_network_id_generator.cpp'),Path('vendor/upstream/raknet/NetworkIDGenerator.cpp'),Path('vendor/upstream/raknet/NetworkIDGenerator.h')]]
...
Path('build/agent-independent/replica-network-id-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/replica-network-id-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
