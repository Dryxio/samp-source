...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30c-three-contract.json'));run=Path('build/cp32-trial30d-one-ob1b');objs={u:COFF(run/(u+'.obj')) for u in ['closure_rpc_connection_rejected','closure_rakclient_disconnect_adjustor']}
...
plan=[('closure_rpc_connection_rejected','?ConnectionRejectedRPCTransfer@@YAXPAURPCParameters@@@Z',0x10540,284),('closure_rakclient_disconnect_adjustor','?Disconnect@R5RakClientDisconnectInterfaceView@@QAEXIE@Z',0x34a40,11)]
...
covered=accepted_code('evidence/checkpoint32/trial30c-three-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==316
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=305,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},typed_layout_note='Vendor original RakClient constructor emits genuine MSVC WNNO thunk for interface offset0xDDE. Full 11 bytes and actual Disconnect target verified. Original slot8 RVAe75d0 selects this thunk. Constructor/vtables not accepted or counted. RPC from025 adapted R5 exact messages.')
assert u32(initial_bytes(p,0xe75d0,4),0)==p.base+0x34a40 # actual interface slot8

Path('build/trial30d/rejection-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30d/rejection-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
