...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/client-router-rpc-contract.json'));run=Path('build/cp32-remote-server-trial1');objs={u:COFF(run/(u+'.obj')) for u in ['closure_remote_driver_update']}
...
plan=[('closure_remote_driver_update','?StateChange@R5RemoteDriverUpdateView@@QAEXEE@Z',0x16270,182),('closure_remote_driver_update','?StoreDriverSync@R5RemoteDriverUpdateView@@QAEXPAUR5RemoteDriverSync@@K@Z',0x17340,242)]
...
covered=accepted_code('evidence/checkpoint32/client-router-rpc-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==424

for row in rows:
 if row['rva'] in (0x1010,0xdfe80,0xf7610):
  old=[x for x in c['regions'] if x['rva']==row['rva'] and x['size']==row['size']];assert old and all(x['sha256']==row['sha256'] for x in old);row['duplicate_previous']=True;row['previous_identical_owners']=[x['unit'] for x in old]
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=0,object_sha256=hashlib.sha256((run/'closure_remote_driver_update.obj').read_bytes()).hexdigest(),source_sha256=hashlib.sha256((run/'client/saco/closure_remote_driver_update.cpp').read_bytes()).hexdigest(),identity='StateChange16270 and StoreDriverSync17340 actual body pair',typed_layout_note='Complete packet63 copied, unconsumed bytes explicitly opaque; all consumed offsets asserted. No view allocation. FindPlayer physical copies receive zero new credit.')
Path('build/agent-textdraw/remote-driver-update-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/agent-textdraw/remote-driver-update-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
