...
rows=json.loads(Path('build/agent-network-b/shaqueue-discovery/discovery-attached.json').read_text())['rows']
...
proposal=json.loads(Path('build/agent-network-b/shaqueue-proposal.json').read_text())
...
out['explicit_owner_transfer']='None; ReliabilityLayer SHA1 and timestamp queues methods reuse existing providers.'
out['unselected_method']='Only real ReliabilityLayer SHA1 and timestamp queues roots and original relocation closure selected; callback implementations are never credited.'
...
Path('build/agent-network-b/shaqueue-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-network-b/shaqueue-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
