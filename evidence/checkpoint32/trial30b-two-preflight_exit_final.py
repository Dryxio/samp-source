...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30-final-contract.json'));run=Path('build/cp32-trial30b-exit');units=['closure_ped_exit_vehicle','closure_remote_exit_vehicle','closure_store_exit_vehicle','closure_rpc_remote_exit_vehicle'];runs={u:(Path('build/cp32-trial30b-exit-rpc') if u=='closure_rpc_remote_exit_vehicle' else run) for u in units};objs={u:COFF(runs[u]/(u+'.obj')) for u in units}
...
plan=[('closure_remote_exit_vehicle', '?ExitVehicle@R5RemoteVehicleExitRequestView@@QAEXXZ', 90672, 53), ('closure_store_exit_vehicle', '?r5MakeActorLeaveCar@@3USCRIPT_COMMAND@@B', 970492, 18), ('closure_rpc_remote_exit_vehicle', '?RemoteExitVehicleTransfer@@YAXPAURPCParameters@@@Z', 71408, 234)]
providers['?GetDistanceFromLocalPlayer@R5RemoteDistanceQueryView@@QAEMXZ']=p.base+0x16120
plan=[x for x in plan if x[0] in ['closure_remote_exit_vehicle','closure_rpc_remote_exit_vehicle']];objs={u:o for u,o in objs.items() if u in ['closure_remote_exit_vehicle','closure_rpc_remote_exit_vehicle']}
...
covered=accepted_code('evidence/checkpoint32/trial30-final-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==308
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=0,object_sha256={u:sha((runs[u]/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((runs[u]/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},identity_callers=[0x116f0],typed_layout_note='Pointer-only remoteviewped1DD and pool3DE/1F8A; realCPlayerPed type source uses real native vehiclehandle queries twice as observed then third acceptedmethod call; wholeSCRIPT_COMMAND18.')
Path('build/agent-client-b/exit-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/agent-client-b/exit-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
