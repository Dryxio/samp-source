...
rows=json.loads(Path('build/agent-independent/datagram-codec-trial2-discovery/discovery-attached.json').read_text())['rows']
byname={r['symbol']:r for r in rows};roots={}
from draft_actor_contract import weak_aliases
units={r['unit'] for r in rows}|{'eval_store_datagram_key','eval_store_datagram_table'}
def objpath(u):return (Path('build/cp32-datagram-codec-trial1') if u.startswith('eval_store_') else run)/(u+'.obj')
rawobjects={u:COFF(objpath(u)) for u in units}
proposal=json.loads(Path('build/agent-independent/datagram-codec-proposal.json').read_text())
for r in proposal['old_unambiguous_roots']:
...
def read_initialized(rva,size):
 try:return p.read(rva,size)
  ...
  return bytes(size)
objects={u:COFF(objpath(u)) for u in units}
...
out['new_unique_code']=len(code-covered);out['already_covered_code']=len(code&covered);out['physical_code_compared']=len(physical_code);out['baseline']=len(covered);out['source025_direct_bytes']=0;out['source025_assisted_adapted_bytes']=194
...
out['explicit_owner_transfer']='None; new R5 datagram codec helper chain only.'
out['unselected_method']='Only four real codec roots and original relocation closure selected; callback implementations are never credited.'
...
out['objects']={unit:hashlib.sha256(objpath(unit).read_bytes()).hexdigest() for unit in objects}
from draft_actor_contract import weak_aliases
out['actual_weak_aliases']={unit:weak_aliases(c) for unit,c in objects.items()}
Path('build/agent-independent/datagram-codec-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/datagram-codec-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
