...
old=json.load(open('config/checkpoint32/remote-server-codec-contract.json'));new=json.load(open('build/actor-contract-draft.json'));prior={(r['unit'],r['rva'],r['size'],r['kind']):r for r in old['regions']}
run=Path('build/cp32-trial30-packets-linked1');pe=PE('private/samp.dll');objs={p.stem:COFF(p) for p in run.glob('*.obj')}
reviews=[json.load(open(p)) for p in ['build/agent-independent/rakclient-convenience-reviewed-manifest.json','build/agent-textdraw/remote-sync-templates-reviewed-manifest.json','build/agent-textdraw/packet-sync-reviewed-manifest.json']]
...
new['review']=dict(old['review']);new['review'].update(scope='Packet/template/state1662 +RakClient555, expected2217 unique.',adaptations='Real inline pool getter context restores full passenger/driver handlers. Complete wire records and actual vendor templates, multipleinheritance RakClient ABI.',reference_credit='Prepared914; trialstarted1303 (driver748 +RakClient555). Direct0250 cumulative22342.',verification='Whole original/linked regions and all COFF/PE targets; ten controls and Actor ABI.',excluded_candidates='No data/CRT/duplicate or unimplemented dependencies credited.')
for name,file in [('itoa_qualification','build/trial30-packets-itoa-qualified.json'),('linker_alias_provider_evidence','build/cp32-trial30-packets-linked1-aliases.json'),('server_bridge_final','build/agent-independent/server-bridge-target-alias-qualification-cp32-trial30-packets-linked1.json'),('logger_deleting_aliases','build/agent-independent/logger-deleting-aliases-cp32-trial30-packets-linked1.json'),('network_tree_deleting_aliases','build/agent-independent/replica-network-id-aliases-qualified.json'),('sha1_deleting_alias_qualification','build/trial30-packets-sha1-deleting-alias-qualified.json'),('mesh_deleting_alias_qualification','build/agent-independent/mesh-deleting-aliases-cp32-trial30-packets-linked1.json'),('chat_navigation_alias','build/agent-textdraw/chat-navigation-linked-alias-review.json')]:new['review'][name]=json.load(open(file))
...
p=Path('config/checkpoint32/trial30-packets-contract.json');p.write_text(json.dumps(new,indent=2)+'\n')
