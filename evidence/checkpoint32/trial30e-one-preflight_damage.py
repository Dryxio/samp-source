...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30d-two-contract.json'));run=Path('build/cp32-trial30e-damage2');objs={u:COFF(run/(u+'.obj')) for u in ['closure_vehicle_damage_update','closure_rpc_vehicle_damage']}
...
plan=[('closure_vehicle_damage_update','?UpdateDamageStatus@R5VehicleDamageView@@QAEXKKE@Z',0xb7ac0,185),('closure_rpc_vehicle_damage','?DamageVehicleRPCTransfer@@YAXPAURPCParameters@@@Z',0x111c0,290)]
...
covered=accepted_code('evidence/checkpoint32/trial30d-two-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==496
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=0,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},typed_layout_note='025 damage update plus RPC adapted to R5 car-or-plane outerguard and fifth tirebyte. Actual accepted subtype106/tire232 callees, vehicle pointer4c/poolvehicles1134/poolactive3074, no newstorage orCRT. Native GTA ECXvehicle calls6a3440 and6b3e90 retained from025 and checked against original completebody.')
Path('build/trial30e/damage-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30e/damage-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
