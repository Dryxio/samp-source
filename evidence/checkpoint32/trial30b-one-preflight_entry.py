...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30-final-contract.json'));run=Path('build/cp32-trial30b-entry');objs={u:COFF(run/(u+'.obj')) for u in ['closure_ped_enter_vehicle', 'closure_remote_enter_vehicle', 'closure_store_enter_vehicle_driver', 'closure_store_enter_vehicle_passenger', 'closure_store_ignore_vehicle_entry', 'closure_remote_distance_query', 'closure_rpc_remote_enter_vehicle']}
...
plan=[('closure_ped_enter_vehicle', '?EnterVehicle@CPlayerPed@@QAEXHH@Z', 705552, 203), ('closure_remote_enter_vehicle', '?EnterVehicle@R5RemoteVehicleEntryRequestView@@QAEXGH@Z', 90528, 143), ('closure_store_enter_vehicle_driver', '?r5SendActorToCarDriver@@3USCRIPT_COMMAND@@B', 970452, 18), ('closure_store_enter_vehicle_passenger', '?r5SendActorToCarPassenger@@3USCRIPT_COMMAND@@B', 970472, 18), ('closure_store_ignore_vehicle_entry', '?r5IgnoreNextVehicleEntry@@3HA', 1381900, 4), ('closure_remote_distance_query', '?GetDistanceFromLocalPlayer@R5RemoteDistanceQueryView@@QAEMXZ', 90400, 45), ('closure_rpc_remote_enter_vehicle', '?RemoteEnterVehicleTransfer@@YAXPAURPCParameters@@@Z', 71120, 280)]
...
covered=accepted_code('evidence/checkpoint32/trial30-final-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==692
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=0,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},identity_callers=[0x115d0],typed_layout_note='Pointer-only views actualped1DD/state10A/pools3DE/nativearray4FB4. WholeSCRIPT_COMMAND18 andBOOL4 stores. Distance constants verified whole.')
Path('build/agent-client-b/entry-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/agent-client-b/entry-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
