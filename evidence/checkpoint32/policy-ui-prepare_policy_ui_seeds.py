...
seeds=json.load(open('config/checkpoint32/rsa-commands-seeds.json'));adds={}
for file in ['build/agent-independent/rakpeer-policy-seeds.json','build/new-vehicle-seeds.json']:
 for u,rows in json.load(open(file)).items():adds.setdefault(u,{}).update(rows)
for r in json.load(open('build/agent-textdraw/cmd-ui-complete-seeds.json')):adds.setdefault(r['unit'],{})[r['symbol']]=r['rva']
...
for u,rows in adds.items():
 o=COFF(Path('build/cp32-policy-ui-ob1')/(u+'.obj'))
...
exports.write_text(s+''.join(lines));Path('config/checkpoint32/policy-ui-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('Added exports',len(lines))
