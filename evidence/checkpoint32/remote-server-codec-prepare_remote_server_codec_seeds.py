...
seeds=json.load(open('config/checkpoint32/client-router-rpc-seeds.json'));adds={}
for file in ['build/agent-independent/datagram-codec-seeds.json','build/agent-independent/rakserver-messages-seeds.json']:
 for u,rows in json.load(open(file)).items():adds.setdefault(u,{}).update(rows)
for file in ['remote-vehicle-entry','remote-passenger-update','remote-driver-update']:
...
for u,rows in adds.items():
 o=COFF(Path('build/cp32-remote-server-codec-ob1-v2')/(u+'.obj'))
...
exports.write_text(s+''.join(lines));Path('config/checkpoint32/remote-server-codec-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('Added exports',len(lines))
