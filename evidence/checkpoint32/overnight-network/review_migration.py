...
migration_reviews=[read(p) for p in spec['manifests']]
migration_expected={(r['unit'],r['rva'],r['size']):r for v in migration_reviews for r in v['regions']}
old=migrate(old,read(pinned(spec['owner_migrations'])),migration_expected)
