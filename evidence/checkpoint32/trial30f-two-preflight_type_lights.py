...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30e-two-contract.json'));run=Path('build/cp32-trial30f-sixth1');objs={'closure_vehicle_type_lights':COFF(run/'closure_vehicle_type_lights.obj')}
...
plan=[('closure_vehicle_type_lights', '?IsTrailerModel@R5VehicleTypeLightsView@@QAEHXZ', 753008, 95), ('closure_vehicle_type_lights', '?IsRCVehicle@R5VehicleTypeLightsView@@QAEHXZ', 753152, 77), ('closure_vehicle_type_lights', '?ToggleLights@R5VehicleTypeLightsView@@QAEXE@Z', 753232, 86)]
...
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},typed_layout_note='Vehicle model and light predicates: original model16 at native22, flags4a8 and428, gtaId44 pointer4c. Source025 RC77/lights43 and adapted toggle86; trailer95/tow43 R5. No new data/CRT.')
Path('build/trial30f/type-lights-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30f/type-lights-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique')
