...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30e-two-contract.json'));run=Path('build/cp32-trial30f-second1');objs={'closure_vehicle_landing_gear':COFF(run/'closure_vehicle_landing_gear.obj')}
...
plan=[('closure_vehicle_landing_gear','?SetLandingGearState@R5VehicleLandingGearView@@QAEXW4eLandingGearState@@@Z',0xb75a0,137),('closure_vehicle_landing_gear','?GetLandingGearState@R5VehicleLandingGearView@@QAE?AW4eLandingGearState@@XZ',0xb7630,92)]
...
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},typed_layout_note='Base source landing-gear methods with original symbolic native GTA bridge at 6CAC20/6CAC70; pointer4c, native landing float9cc, complete constants, real subtype provider. No new storage CRT.')
Path('build/trial30f/landing-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30f/landing-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique')
