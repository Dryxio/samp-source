...
old=json.load(open('config/checkpoint32/chat-resources-contract.json'));new=json.load(open('build/actor-contract-draft.json'));prior={(r['rva'],r['size'],r['kind']):r for r in old['regions']}
run=Path('build/cp32-btree-scoreboard-linked1');pe=PE('private/samp.dll');objs={p.stem:COFF(p) for p in run.glob('*.obj')}
reviews=[json.load(open('build/agent-independent/reliability-ack-reviewed-manifest.json')),json.load(open('build/scoreboard-actions-reviewed-manifest.json'))]
...
unused_compiler_alias={'??$RangeNodeComp@G@DataStructures@@YAHABGABU?$RangeNode@G@0@@Z':'?RangeNodeComp@DataStructures@@YAHABGABU?$RangeNode@G@1@@Z'}
assert new['weak_aliases']['closure_vendor_reliability_ack_family']==unused_compiler_alias
assert all(f['symbol'] not in set(unused_compiler_alias)|set(unused_compiler_alias.values()) for r in new['regions'] if r['unit']=='closure_vendor_reliability_ack_family' for f in r['fixups'])
assert all(not new['weak_aliases'][u] for v in reviews for u in v['seeds'] if u!='closure_vendor_reliability_ack_family')
...
new['review']=dict(old['review']);new['review'].update(scope='BPlusTree removal and RangeList serialization2393 including EH42, scoreboard actions441 including EH21.',adaptations='Normal vendor code with explicit instantiation of real Serialize/Deserialize; actual original callers establish their identities. Insert505 still has stack-temporary differences after bounded context trial and is excluded with SendAcknowledgement22. Three scoreboard methods normal C++; retain actual ListBox pointer snapshot between GetSelectedIndex/GetItem to match ClickSelected282. True partial view, never allocate unknown ListBox. Whole actual RPC23 scalar4.',verification='All complete functions, EH/xdata maps and RPC data, every COFF/PE binding and true provider. No data slicing, instruction copying or helper homologue credit. Ten controls and actor ABI required. Actual whole scoreboard TU Ob2 for original Hide inlining; other profile owners retained. No headers changed, cached unchanged objects reused.',reference_credit='Direct0250; cumulative21126.',initialization_limitation='Only real queue/tree operations and scoreboard actions; no live networking or game/UI execution, no whole DLL claim.',excluded_candidates='Unmatched Insert505/SendAck22 unseeded. Scoreboard ListBox constructor and update family unclaimed; existing old five pending remain.')
new['review']['compiler_weak_alias_note']='Exact compiler-generated RangeNodeComp weak alias recorded in ACK TU; no accepted fixup from that unit references either spelling. Existing lifecycle comparator definition/caller remains fully verified unchanged. No alias implementation or new coverage credited.'
new['review']['linker_alias_provider_evidence']=json.load(open('build/cp32-btree-scoreboard-linked1-aliases.json'))
p=Path('config/checkpoint32/btree-scoreboard-contract.json');p.write_text(json.dumps(new,indent=2)+'\n')
