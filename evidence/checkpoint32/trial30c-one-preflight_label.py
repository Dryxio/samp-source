...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30b-two-contract.json'));run=Path('build/cp32-trial30c-label-ack-owner');objs={u:COFF(run/(u+'.obj')) for u in ['closure_rpc_text_label_create']}
...
plan=[('closure_rpc_text_label_create','?CreateTextLabelRPCTransfer@@YAXPAURPCParameters@@@Z',0xf720,421)]
...
covered=accepted_code('evidence/checkpoint32/trial30b-two-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==442
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=0,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},typed_layout_note='Same complete R5CompleteLabelRecord and pool declarations as accepted closure_label_pool_new with size/offset assertions; existing CNetGame pools3DE/label18. Byte-serialized bool; no new stores or CRT initialization.')
Path('build/trial30c/client-agent/text-label-create-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30c/client-agent/text-label-create-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
