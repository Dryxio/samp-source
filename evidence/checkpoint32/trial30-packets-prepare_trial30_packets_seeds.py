...
seeds=json.load(open('config/checkpoint32/remote-server-codec-seeds.json'));adds={}
for file in ['build/agent-independent/rakclient-convenience-seeds.json']:
 for u,rows in json.load(open(file)).items():adds.setdefault(u,{}).update(rows)
for file in ['remote-sync-templates','packet-sync']:
...
for u,rows in adds.items():
 o=COFF(Path('build/cp32-trial30-packets-ob1')/(u+'.obj'))
...
exports.write_text(s+''.join(lines));Path('config/checkpoint32/trial30-packets-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('Added exports',len(lines))
