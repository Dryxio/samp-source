...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30e-two-contract.json'));run=Path('build/cp32-trial30f-fifth1');objs={u:COFF(run/(u+'.obj')) for u in ['closure_rpc_request_class']}
...
plan=[('closure_rpc_request_class','?RequestClassRPCTransfer@@YAXPAURPCParameters@@@Z',0xffd0,205),('closure_rpc_request_class','?HandleClassSelectionOutcome@R5ClassOutcomeLocalView@@QAEXH@Z',0x40e0,58)]
...
covered=accepted_code('evidence/checkpoint32/trial30e-two-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==284
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=0,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},typed_layout_note='Source025 requestclass/outcome adapted R5 wire46 and unconditional local player; asserted packed offsets. Existing accepted SetSpawnInfo3be0 and ped providers; no new storage or CRT.')
Path('build/trial30f/client-agent/request-class-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30f/client-agent/request-class-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
