...
seeds=json.load(open('config/checkpoint32/packet-registration-seeds.json'));adds={}
for file in ['build/agent-independent/rakpeer-send-handshake-seeds.json','build/final-toggles-seeds.json']:
 for u,rows in json.load(open(file)).items():adds.setdefault(u,{}).update(rows)
for file in ['build/agent-textdraw/font-setup-seeds.json','build/agent-textdraw/transfer-layout-seeds.json']:
...
for u,rows in adds.items():
 o=COFF(Path('build/cp32-send-font-ob1')/(u+'.obj'))
...
exports.write_text(s+''.join(lines));Path('config/checkpoint32/send-font-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('Added exports',len(lines))
