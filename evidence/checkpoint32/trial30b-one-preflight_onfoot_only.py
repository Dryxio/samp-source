...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30-final-contract.json'));run=Path('build/cp32-trial30b-one-ob1');objs={u:COFF(run/(u+'.obj')) for u in ['closure_remote_onfoot_update']}
...
plan=[('closure_remote_onfoot_update','?StoreOnFootSync@R5RemoteOnFootUpdateView@@QAEXPAUR5RemoteOnFootSync@@K@Z',0x17260,221),('closure_remote_onfoot_update','?EnsureVehicleExit@R5RemoteOnFootUpdateView@@QAEXXZ',0x148f0,82),('closure_remote_onfoot_update','?IsExitingVehicle@R5PedExitTaskView@@QAEHXZ',0xac640,70)]
...
covered=accepted_code('evidence/checkpoint32/trial30-final-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==373
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=0,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},identity_callers=[0xa740],typed_layout_note='Pointer-only partial views no allocation; complete68-byte sync; offsets asserted; originalA740 caller establishes identity.')
Path('build/agent-client-b/onfoot-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/agent-client-b/onfoot-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
