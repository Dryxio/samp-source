...
run=Path('build/cp32-send-font-linked1');unit='closure_vendor_sha1';o=COFF(run/(unit+'.obj'));p=PE(Path('private/samp.dll'));q=PE(run/'closure.dll');maps=map_symbols(run/'closure.map');manifest=json.loads(Path('build/sha1-reviewed-manifest.json').read_text());e='??_ECSHA1@@UAEPAXI@Z';g='??_GCSHA1@@UAEPAXI@Z';v='??_7CSHA1@@6B@'
...
Path('build/send-font-sha1-deleting-alias-qualified.json').write_text(json.dumps(x,indent=2)+'\n');print('PASS real SHA1 E->G, full VFT4 +G30 both images')
