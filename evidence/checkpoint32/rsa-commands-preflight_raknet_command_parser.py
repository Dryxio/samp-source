...
rows=[r for r in json.loads((run/'discovery-attached.json').read_text())['rows'] if 'raknet_command_parser' in r['unit'].lower()]
...
out['explicit_owner_transfer']='None; new RakNetCommandParser sole owner.'
out['unselected_method']='Seven actual parser methods identified by full callers, own VFT and literal identities. No concrete RakPeerInterface implementation credit.'
...
Path('build/agent-independent/raknet-command-parser-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/raknet-command-parser-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
