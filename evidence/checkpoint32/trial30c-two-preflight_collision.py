...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30b-two-contract.json'));run=Path('build/cp32-trial30c-vehicle-collision');objs={u:COFF(run/(u+'.obj')) for u in ['closure_rpc_vehicle_collision']}
...
plan=[('closure_rpc_vehicle_collision','?SetVehicleCollisionRPCTransfer@@YAXPAURPCParameters@@@Z',0xea20,95)]
...
Path('build/trial30c/client-agent/vehicle-collision-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30c/client-agent/vehicle-collision-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
