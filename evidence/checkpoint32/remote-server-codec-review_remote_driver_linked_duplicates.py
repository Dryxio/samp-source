...
run='cp32-remote-server-codec-linked2';d=Path('build')/run;p=PE(Path('private/samp.dll'));linked=PE(d/'closure.dll');base=json.loads(Path('config/checkpoint32/client-router-rpc-contract.json').read_text());draft=json.loads(Path('build/agent-textdraw/remote-driver-update-reviewed-manifest.json').read_text());assert draft['run']=='cp32-remote-server-trial1'
unit='closure_remote_driver_update';oldunit='closure_player_lifetime';rvas={0x1010,0xdfe80,0xf7610};new=[r for r in draft['regions'] if r['unit']==unit and r['rva'] in rvas];old=[r for r in base['regions'] if r['unit']==oldunit and r['rva'] in rvas];assert len(new)==len(old)==3
...
for owner,regions in [(oldunit,old),(unit,new)]:
 ...
 for r in regions:
  sec=o.sections[r['section']-1];assert r.get('offset',0)==0 and r['size']==sec['size'];raw=bytearray(sec['bytes']);actual=bytearray(sec['bytes']);bindings=[];lf=set();of=set()
  ...
  if owner==unit:rows.append(dict(unit=owner,anchor=r['anchor'],section=r['section'],rva=r['rva'],size=r['size'],kind=r['kind'],anchor_offset=r.get('anchor_offset',0),sha256=r['sha256'],object_sha256=hashlib.sha256((d/(owner+'.obj')).read_bytes()).hexdigest(),bindings=bindings,selected_linked_owner=oldunit+'.obj',linked_va=linked_bases[r['rva']],new_unique_code=0))
ac=accepted_code('evidence/checkpoint32/client-router-rpc-acceptance.json');assert all(set(range(r['rva'],r['rva']+r['size']))<=ac for r in new if r['kind']=='code')
