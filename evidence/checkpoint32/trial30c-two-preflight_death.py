...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30b-two-contract.json'));run=Path('build/cp32-trial30c-death');objs={u:COFF(run/(u+'.obj')) for u in ['closure_ped_extinguish_fire','closure_ped_set_dead','closure_remote_handle_death','closure_rpc_player_death']}
...
plan=[('closure_ped_extinguish_fire','?ExtinguishFire@R5DeathPedView@@QAEXXZ',0xad0f0,95),('closure_ped_set_dead','?SetDead@R5DeathPedView@@QAEXXZ',0xaffd0,171),('closure_remote_handle_death','?HandleDeath@R5RemoteDeathView@@QAEXXZ',0x17570,61),('closure_rpc_player_death','?WorldPlayerDeathRPCTransfer@@YAXPAURPCParameters@@@Z',0x10d40,189)]
...
covered=accepted_code('evidence/checkpoint32/trial30b-two-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==537
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=516,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},typed_layout_note='R5 pointer views: ped GTA44/native2A4/player2B0, remote state10A/ped1DD, pool1F8A. CFire5393F0 symbolic native member-call asm from source025; no new stores or CRT initialization.')
Path('build/trial30c/client-agent/death-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30c/client-agent/death-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
