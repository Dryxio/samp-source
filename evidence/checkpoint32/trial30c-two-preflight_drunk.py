...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30b-two-contract.json'));run=Path('build/cp32-trial30c-game-batch');objs={u:COFF(run/(u+'.obj')) for u in ['closure_ped_drunk_level','closure_rpc_drunk_level']}
...
plan=[('closure_ped_drunk_level','?SetDrunkLevel@R5DrunkLevelPedView@@QAEXH@Z',0xadfb0,13),('closure_rpc_drunk_level','?SetPlayerDrunkLevelRPCTransfer@@YAXPAURPCParameters@@@Z',0x18db0,158)]
...
covered=accepted_code('evidence/checkpoint32/trial30b-two-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==192
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=0,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},typed_layout_note='Pointer-only R5 ped int field2C9; existing actual CPlayerPed.field_2C9 corroborates width and offset; no new stores or CRT initialization.')
Path('build/trial30c/client-agent/drunk-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30c/client-agent/drunk-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
