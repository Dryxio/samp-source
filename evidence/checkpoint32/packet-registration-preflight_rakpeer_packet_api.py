...
rows=json.loads(Path('build/agent-independent/packet-api-trial1-discovery/discovery-attached.json').read_text())['rows']
...
proposal=json.loads(Path('build/agent-independent/rakpeer-packet-api-proposal.json').read_text())
...
roots['?GetWriteOffset@BitStream@RakNet@@QBEHXZ']=0x1F720
...
out['explicit_owner_transfer']='None; new packet API. Existing remote-state/RPCMap/BitStream source owners retained.'
out['unselected_method']='Only real packet API roots and original relocation closure selected; callback implementations are never credited.'
...
Path('build/agent-independent/rakpeer-packet-api-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/rakpeer-packet-api-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
