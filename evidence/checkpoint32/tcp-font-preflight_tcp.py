...
rows=[r for r in json.loads((run/'discovery-attached.json').read_text())['rows'] if r['unit'] in ['eval_vendor_tcp_interface','eval_vendor_rak_sleep']]
byname={r['symbol']:r for r in rows};roots={}
for r in rows:
 n=r['symbol']
 if '@TCPInterface@@' in n or n.startswith(('??0TCPInterface@@','??1TCPInterface@@','?UpdateTCPInterfaceLoop@@','?RakSleep@@')):
  assert len(r['candidates'])==1,(n,r['candidates']);roots[n]=r['candidates'][0]['rva']
...
out['source_files']=[dict(path=str(f),sha256=hashlib.sha256(f.read_bytes()).hexdigest()) for f in [Path('client/saco/eval_vendor_tcp_interface.cpp'),Path('client/saco/eval_vendor_rak_sleep.cpp'),Path('vendor/upstream/raknet/TCPInterface.cpp'),Path('vendor/upstream/raknet/TCPInterface.h'),Path('vendor/upstream/raknet/SingleProducerConsumer.h'),Path('vendor/upstream/raknet/RakSleep.cpp')]]
...
Path('build/agent-independent/tcp-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/tcp-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
