...
RUN=Path('build/cp32-overnight-probe-lzma-encoder');BASE='overnight-network-one'
units=['closure_vendor_lzma_encoder','closure_vendor_lzma_matchfinder'];objs={u:COFF(RUN/(u+'.obj')) for u in units};pe=PE('private/samp.dll')
dis=json.load(open(RUN/'discovery-attached.json'))['rows'];known={};roots=[]
for r in dis:
...
known[('closure_vendor_lzma_encoder','_LzmaEnc_Finish')]=0xc3590
...
providers['__aullshr']=0x100cbaf0
...
result=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',baseline=BASE,run=str(RUN),roots=roots,regions=regions,new_unique_code=len(code-covered),object_sha256={u:sha((RUN/(u+'.obj')).read_bytes()) for u in units},source_sha256={u:sha((RUN/'client/saco'/(u+'.cpp')).read_bytes()) for u in units},actual_weak_aliases={u:{} for u in units},provenance='Igor Pavlov LZMA SDK16.04 public domain source from https://www.7-zip.org/a/lzma1604.7z. Canonical units are unchanged source/header amalgamations, _7ZIP_ST selects original single-thread layout. Full unique fingerprints and actual cross-calls anchor every code region; Ambiguous unreferenced Finish no-op remains outside coverage. No original instruction copying, masking, or fake providers. All constants and actual targets checked. Link and acceptance remain required.')
Path('evidence/checkpoint32/overnight-lzma').mkdir(exist_ok=True)
Path('evidence/checkpoint32/overnight-lzma/encoder-reviewed-manifest.json').write_text(json.dumps(result,indent=2)+'\n')
