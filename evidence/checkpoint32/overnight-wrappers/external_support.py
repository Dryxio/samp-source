...
def extend(base,extra):
    ...
    for name,e in extra.items():
        ...
        if e['kind']=='crt':
            ...
            require(e['size']==4 and import_slots(pe).get(e['reference_va'])==tuple(e['import']),'Direct IAT identity or size differs')
            require(not any(rva<=a<rva+4 for a in pe.relocations),'Unexpected IAT relocations')
            e={k:v for k,v in e.items() if k not in ('size','sha256')}
        else:
