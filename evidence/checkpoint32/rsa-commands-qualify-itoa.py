...
run='cp32-rsa-commands-linked1';p=PE('private/samp.dll');q=PE('build/'+run+'/closure.dll');m=Path('build/'+run+'/closure.map').read_text();inv=json.load(open('config/checkpoint31/inventory.json'))
rows=[];mapping={}
...
Path('build/rsa-commands-itoa-qualified.json').write_text(json.dumps(dict(status='PINNED_CRT_PROVIDER_FULL_BYTES_QUALIFIED',run=run,linked_sha256=sha(q.data),new_source_credit=0,code=rows),indent=2)+'\n');print('PASS __itoa42 + true _xtoa62, no source credit')
