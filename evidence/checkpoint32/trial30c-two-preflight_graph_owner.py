...
p=PE(Path('private/samp.dll'));run=Path('build/trial30c/network-agent/graph-owner-discovery')
...
roots={n:329184 for n in byname if 'GenerateDisjktraMatrix' in n}
...
out['source_files']=[dict(path=str(f),sha256=hashlib.sha256(f.read_bytes()).hexdigest()) for f in [Path('client/saco/closure_vendor_graph_algorithms.cpp')]]
Path('build/trial30c/network-agent/graph-owner-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/trial30c/network-agent/graph-owner-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
