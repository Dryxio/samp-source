...
rows=json.loads(Path('build/agent-independent/rakserver-messages-trial1-discovery/discovery-attached.json').read_text())['rows']
...
proposal=json.loads(Path('build/agent-independent/rakserver-messages-proposal.json').read_text())
...
out['explicit_owner_transfer']='None; new RakServer message methods.'
out['unselected_method']='Only real RakServer message roots and original relocation closure selected; callback implementations are never credited.'
...
Path('build/agent-independent/rakserver-messages-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/rakserver-messages-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
