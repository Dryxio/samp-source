...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/disconnect-selection-contract.json'));run=Path('build/cp32-rpc-ped-vehicle-control-trial2');objs={u:COFF(run/(u+'.obj')) for u in ['closure_rpc_ped_vehicle_control','closure_vehicle_gta_id_lookup']}
...
plan=[('closure_rpc_ped_vehicle_control','?ScrPutPlayerInVehicleTransfer@@YAXPAURPCParameters@@@Z',0x19600,261),('closure_rpc_ped_vehicle_control','?ScrTogglePlayerControllableTransfer@@YAXPAURPCParameters@@@Z',0x1a290,156),('closure_vehicle_gta_id_lookup','?FindGtaIDFromID@R5VehicleGtaIdPoolView@@QAEHH@Z',0x1ebc0,32)]
...
while todo:
 ...
 anchor=next(x for x in o.symbols.values() if x['section']==si and x['name']!=sec['name']);rows.append(dict(unit=u,section=si,anchor=anchor['name'],rva=r,size=sec['size'],kind='code' if sec['flags']&0x20 else 'data',sha256=sha(ref),bindings=bindings))
covered=accepted_code('evidence/checkpoint32/disconnect-selection-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==491
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=0)
Path('build/root-rpc-control-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/root-rpc-control-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
