...
rows=json.loads(Path('build/agent-independent/rakpeer-options-trial1-discovery/discovery-attached.json').read_text())['rows']
...
proposal=json.loads(Path('build/agent-independent/rakpeer-options-proposal.json').read_text())
...
out['explicit_owner_transfer']='None; RakPeer options methods reuse existing providers.'
out['unselected_method']='Only real RakPeer options roots and original relocation closure selected; callback implementations are never credited.'
...
Path('build/agent-independent/rakpeer-options-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/rakpeer-options-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
