...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/trial30e-two-contract.json'));run=Path('build/cp32-trial30f-fourth1');objs={'closure_vehicle_marker_process':COFF(run/'closure_vehicle_marker_process.obj')}
...
plan=[('closure_vehicle_marker_process','?ProcessMarkers@R5VehicleMarkersView@@QAEXXZ',0xb76e0,337)]
...
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,object_sha256={u:sha((run/(u+'.obj')).read_bytes()) for u in objs},source_sha256={u:sha((run/('client/saco/'+u+'.cpp')).read_bytes()) for u in objs},typed_layout_note='025 ProcessMarkers with original R5 colors1006/1004 and actual entity distance/vehicle occupied providers. Pointer4c, objective64, special65, markerb4; complete SCM descriptors and200float; no new storage CRT.')
Path('build/trial30f/markers-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/trial30f/markers-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique')
