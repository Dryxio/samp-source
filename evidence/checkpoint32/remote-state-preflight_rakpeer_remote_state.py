...
rows=json.loads(Path('build/agent-independent/remote-state-trial1-discovery/discovery-attached.json').read_text())['rows']
...
# The original complete class source has these semantic roots. Tiny overlapping
# bodies are anchored by their real direct targets, not shape-only candidates.
import re
for row in rows:
 n=row['symbol']
 if re.match(r'^\?[^?@][^@]*@RakPeer@@',n) and len(row['candidates'])==1:
  roots[n]=row['candidates'][0]['rva']
roots['?GetIndexFromPlayerID@RakPeer@@UAEHUPlayerID@@@Z']=0x3E330
# RPCMap complete constructor and simple registration are actual source methods;
# getters are selected only once original call sites establish their identities.
for n,rva in {'??0RPCMap@@QAE@XZ':0x51630,
 '?AddIdentifierWithFunction@RPCMap@@QAEXEPAX_N@Z':0x516B0,
 '?GetIndexFromFunctionName@RPCMap@@QAEEE@Z':0x516A0,
 '?GetNodeFromIndex@RPCMap@@QAEPAURPCNode@@E@Z':0x51680,
 '??1RPCMap@@QAE@XZ':0x516E0}.items():
 assert any(c['rva']==rva for c in byname[n]['candidates']);roots[n]=rva
...
out['explicit_owner_transfer']='None; source full existing headers, new remote-state/RPCMap/providers only.'
out['unselected_method']='RPCMap homologue getters and trivial destructor require their actual original call sites before additional roots.'
...
Path('build/agent-independent/rakpeer-remote-state-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/rakpeer-remote-state-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
