...
old=json.load(open('config/checkpoint32/client-router-rpc-contract.json'));new=json.load(open('build/actor-contract-draft.json'));prior={(r['unit'],r['rva'],r['size'],r['kind']):r for r in old['regions']}
run=Path('build/cp32-remote-server-codec-linked2');pe=PE('private/samp.dll');objs={p.stem:COFF(p) for p in run.glob('*.obj')}
reviews=[json.load(open(p)) for p in ['build/agent-independent/datagram-codec-reviewed-manifest.json','build/agent-independent/rakserver-messages-reviewed-manifest.json']+['build/agent-textdraw/'+n+'-reviewed-manifest.json' for n in ['remote-vehicle-entry','remote-passenger-update','remote-driver-update']]]
...
for r in new['regions']:
 ...
 r['implementation']='Normal C++ remote vehicle sync, datagram codec, RakServer messages; complete source regions and original/linked targets'
...
new['review']=dict(old['review']);new['review'].update(scope='Remote899 +codec210 +RakServer1149; expected2258 unique.',adaptations='Complete sync records and pointer-only remote views, true inline getter. Vendor RakServer Receive adapted to original reduced behavior, complete real class. Codec split ABI and real256-byte table/WORD key.',reference_credit='Direct025 new0 cumulative22342; codec194 assisted/adapted, remote hybrid.',verification='Complete original/linked regions and COFF/PE; ten negative controls and bounded Actor ABI.',excluded_candidates='SendTo137 excluded: global output extent unresolved. Three inherited pending excluded.')
for name,file in [('itoa_qualification','build/remote-server-codec-itoa-qualified.json'),('linker_alias_provider_evidence','build/cp32-remote-server-codec-linked2-aliases.json'),('server_bridge_final','build/agent-independent/server-bridge-target-alias-qualification-cp32-remote-server-codec-linked2.json'),('logger_deleting_aliases','build/agent-independent/logger-deleting-aliases-cp32-remote-server-codec-linked2.json'),('network_tree_deleting_aliases','build/agent-independent/replica-network-id-aliases-qualified.json'),('sha1_deleting_alias_qualification','build/remote-server-codec-sha1-deleting-alias-qualified.json'),('mesh_deleting_alias_qualification','build/agent-independent/mesh-deleting-aliases-cp32-remote-server-codec-linked2.json'),('chat_navigation_alias','build/agent-textdraw/chat-navigation-linked-alias-review.json')]:new['review'][name]=json.load(open(file))
...
p=Path('config/checkpoint32/remote-server-codec-contract.json');p.write_text(json.dumps(new,indent=2)+'\n')
