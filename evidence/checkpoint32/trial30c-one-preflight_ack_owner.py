...
p=PE(Path('private/samp.dll'));run=Path('build/trial30c/network-agent/ack-owner-discovery')
rows=[r for r in json.loads((run/'discovery-attached.json').read_text())['rows'] if r['unit']=='closure_vendor_reliability_ack_family']
byname={r['symbol']:r for r in rows}
roots={'?SendAcknowledgementPacket@ReliabilityLayer@@AAEXG_J@Z':296704, '?RemovePacketFromResendListAndDeleteOlderReliableSequenced@ReliabilityLayer@@AAEIG_J@Z':0x486b0,'?Serialize@?$RangeList@G@DataStructures@@QAEIPAVBitStream@RakNet@@H_N@Z':0x46f40,'?Deserialize@?$RangeList@G@DataStructures@@QAE_NPAVBitStream@RakNet@@@Z':0x47d20}
...
for unit,c in objects.items():
 for si,rva in sections[unit].items():
  ...
  for j in range(sec['reloc_count']):
   ...
   assert target==actual,(name,hex(target),hex(actual));assert ((rva+off) in p.relocations)==(k==6 and name!='__except_list');struct.pack_into('<I',raw,off,value);bs.append(dict(symbol=name,site_rva=rva+off,target_va=target,kind=k))
...
proof_path='evidence/checkpoint32/trial30b-two-acceptance.json'
...
out['source_files']=[dict(path=str(f),sha256=hashlib.sha256(f.read_bytes()).hexdigest()) for f in [Path('client/saco/closure_vendor_reliability_ack_family.cpp')]]
Path('build/trial30c/network-agent/ack-owner-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/trial30c/network-agent/ack-owner-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
