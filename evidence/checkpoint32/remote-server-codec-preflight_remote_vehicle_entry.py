...
p=PE('private/samp.dll');c=json.load(open('config/checkpoint32/client-router-rpc-contract.json'));run=Path('build/cp32-remote-vehicle-entry-trial1');objs={u:COFF(run/(u+'.obj')) for u in ['closure_remote_vehicle_entry']}
...
plan=[('closure_remote_vehicle_entry','?EnsureVehicleEntry@R5RemoteVehicleEntryView@@QAEXXZ',0x14800,238)]
...
covered=accepted_code('evidence/checkpoint32/client-router-rpc-acceptance.json');new={a for r in rows if r['kind']=='code' for a in range(r['rva'],r['rva']+r['size'])}-covered;assert len(new)==238
out=dict(status='PASS_FULL_SOURCE_REGIONS_ALL_COFF_PE_TARGETS',run=run.name,baseline=len(covered),new_unique_code=len(new),regions=rows,source025_direct=0,object_sha256=hashlib.sha256((run/'closure_remote_vehicle_entry.obj').read_bytes()).hexdigest(),source_sha256=hashlib.sha256((run/'client/saco/closure_remote_vehicle_entry.cpp').read_bytes()).hexdigest(),identity_callers=[0x17340,0x17440],typed_layout_note='Pointer-only partial view, no allocation; offsets asserted4F/B0/10B/1DD/1E7; same pool and net views as accepted RPC owners; no standalone inline-helper credit.')
Path('build/agent-textdraw/remote-vehicle-entry-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n');Path('build/agent-textdraw/remote-vehicle-entry-seeds.json').write_text(json.dumps(seeds,indent=2)+'\n');print('PASS',len(rows),'whole regions',len(new),'unique',sum(len(r['bindings']) for r in rows),'fixups')
