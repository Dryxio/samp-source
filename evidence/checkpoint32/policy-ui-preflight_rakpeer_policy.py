...
rows=[r for r in json.loads((run/'discovery-attached.json').read_text())['rows'] if r['unit'] in ['eval_vendor_rakpeer_policy','eval_vendor_simple_mutex']]
...
proposal=json.loads(Path('build/agent-independent/rakpeer-policy-proposal.json').read_text())
for root in proposal['roots']:
 n=root['symbol'];rva=root['rva'];assert n in byname
 assert any(c['rva']==rva for c in byname[n]['candidates']),(n,'fresh mismatch')
 roots[n]=rva
...
out['explicit_owner_transfer']='None; new RakPeer policy methods and SimpleMutex providers. Existing RSA/Huffman dependencies retain accepted owners.'
out['unselected_method']='Unreferenced mutex lifetime methods and unrelated emitted template methods are not selected without original identity.'
...
Path('build/agent-independent/rakpeer-policy-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/rakpeer-policy-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
