...
UNIT = "eval_vendor_system_address_list"
OBJ = RUN / (UNIT + ".obj")
SRC = Path("build/cp32-trial30e-two-ob1/client/saco/eval_vendor_system_address_list.cpp")
...
plan = [("??0SystemAddressList@@QAE@UPlayerID@@@Z",347680,89)]
...
assert len(new_unique) >=89, len(new_unique)
...
manifest = dict(
    ...
    identity="Actual SystemAddressList constructor in existing vendor source; whole constructor89, EH handler18 and full unwind data36, all original COFF and PE targets validated.",
    crt_initialization="object .drectve requests LIBCMT/OLDNAMES; no CRT initializer section is in the selected closure",
)
(OUTDIR / "address-ctor-reviewed-manifest.json").write_text(json.dumps(manifest, indent=2) + "\n")
(OUTDIR / "address-ctor-seeds.json").write_text(json.dumps(seeds, indent=2) + "\n")
