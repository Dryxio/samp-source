...
old=json.load(open('config/checkpoint32/send-font-contract.json'));new=json.load(open('build/actor-contract-draft.json'));prior={(r['unit'],r['rva'],r['size'],r['kind']):r for r in old['regions']}
run=Path('build/cp32-disconnect-selection-linked1');pe=PE('private/samp.dll');objs={p.stem:COFF(p) for p in run.glob('*.obj')}
reviews=[json.load(open(p)) for p in ['build/agent-independent/rakpeer-disconnect-requests-reviewed-manifest.json','build/agent-textdraw/selection-ped-reviewed-manifest.json','build/agent-textdraw/vehicle-selection-reviewed-manifest.json']]
...
for r in new['regions']:
 ...
 r['implementation']='Normal C++ complete disconnect/request and actual ped/vehicle selection with whole stores, original and linked targets'
...
new['review']=dict(old['review']);new['review'].update(scope='Disconnect1862 +ped559 +vehicle selection965, expected3386 unique.',adaptations='Actual complete RemoteSystem3177 destructor and queue owners; no RakPeer allocation. Ped methods use full real types and whole script descriptors18. Vehicle selection uses actual184 factory and real native controls, complete globals, no fake class allocation.',reference_credit='Direct025 new20 dispatcher, cumulative22342; selection945 adapted and ped559 base.',verification='Only changed sources compile, full original/linked code/data/EH and COFF/PE targets. All previous regions retained. Ten negative controls and Actor ABI required.',excluded_candidates='Three inherited pending and any unimplemented native startup bodies excluded; no CRT/data/duplicate credit.')
for name,file in [('itoa_qualification','build/disconnect-selection-itoa-qualified.json'),('linker_alias_provider_evidence','build/cp32-disconnect-selection-linked1-aliases.json'),('server_bridge_final','build/agent-independent/server-bridge-target-alias-qualification-cp32-disconnect-selection-linked1.json'),('logger_deleting_aliases','build/agent-independent/logger-deleting-aliases-cp32-disconnect-selection-linked1.json'),('network_tree_deleting_aliases','build/agent-independent/replica-network-id-aliases-qualified.json'),('sha1_deleting_alias_qualification','build/disconnect-selection-sha1-deleting-alias-qualified.json'),('mesh_deleting_alias_qualification','build/agent-independent/mesh-deleting-aliases-cp32-disconnect-selection-linked1.json'),('chat_navigation_alias','build/agent-textdraw/chat-navigation-linked-alias-review.json')]:new['review'][name]=json.load(open(file))
new['review']['disconnect_scope']=json.load(open('build/agent-independent/rakpeer-disconnect-requests-scope-reviewed.json'))
...
p=Path('config/checkpoint32/disconnect-selection-contract.json');p.write_text(json.dumps(new,indent=2)+'\n')
