...
rows=json.loads(Path('build/agent-independent/send-handshake-trial2-discovery/discovery-attached.json').read_text())['rows']
...
proposal=json.loads(Path('build/agent-independent/rakpeer-send-handshake-proposal.json').read_text())
...
roots.update({'?GetWriteOffset@BitStream@RakNet@@QBEHXZ': 128800, '??1?$RSACrypt@$$BY07I@big@@QAE@XZ': 248944, '?reset@?$RSACrypt@$$BY07I@big@@QAEXXZ': 241872, '??0?$RSACrypt@$$BY07I@big@@QAE@XZ': 248928, '?setPublicKey@?$RSACrypt@$$BY07I@big@@QAEXIAAY07I@Z': 265104, '?getPublicKey@?$RSACrypt@$$BY07I@big@@QAEXAAIAAY07I@Z': 241840})
...
out['explicit_owner_transfer']='None; new send/handshake source methods. Existing providers retained.'
out['unselected_method']='Only real send/handshake roots and original relocation closure selected; callback implementations are never credited.'
...
Path('build/agent-independent/rakpeer-send-handshake-reviewed-manifest.json').write_text(json.dumps(out,indent=2)+'\n')
Path('build/agent-independent/rakpeer-send-handshake-seeds.json').write_text(json.dumps(out['seeds'],indent=2)+'\n')
