...
spec=json.load(open(sys.argv[1]));old=json.load(open('config/checkpoint32/'+spec['previous']+'-contract.json'));new=json.load(open(spec['draft_file']))
from aggregate_support import aggregate
from external_support import extend
from integration_pipeline import pinned,read
old=extend(old,read(pinned(spec['reviewed_externals'])))
aggregation_expected={(r['unit'],r['rva'],r['size']):r for file in spec['manifests'] for r in read(file)['regions']}
old=aggregate(old,read(pinned(spec['data_aggregation'])),aggregation_expected)
prior={(r['unit'],r['rva'],r['size'],r['kind']):r for r in old['regions']}
