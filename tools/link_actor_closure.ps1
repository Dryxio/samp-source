...
Push-Location $Out
try {
 & link.exe /nologo /dll /incremental:no /opt:ref /opt:noicf /base:0x10000000 /map:closure.map /out:closure.dll /def:closure-exports.def @Objects kernel32.lib user32.lib wsock32.lib d3dx9.lib
 if($LASTEXITCODE -ne 0) {throw 'Link failed'}
} finally {Pop-Location}
$Hashes=@{}
Get-ChildItem $Out -File | ForEach-Object {$Hashes[$_.Name]=(Get-FileHash $_.FullName).Hash.ToLowerInvariant()}
$Sdk=@{}
foreach($Name in @('libcmt.lib','libcpmt.lib','oldnames.lib','kernel32.lib','user32.lib','uuid.lib','wsock32.lib','d3dx9.lib')) {
