...
def main():
    gate=Gate();result=gate.verify()
    sys.path.insert(0,str(ROOT/'tests/checkpoint32'))
    from actor_closure_emulate import exercise
    emulation=exercise(gate)
    test=subprocess.run([sys.executable,'-m','unittest','discover','-s','tests/checkpoint32','-p','test_actor_closure_gate.py','-v'],cwd=ROOT,stdout=subprocess.PIPE,stderr=subprocess.STDOUT,text=True)
