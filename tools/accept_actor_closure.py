...
def main():
    ...
    report=dict(previous_proofs={p:sha((ROOT/p).read_bytes()) for p in args.previous},result='PASS',checkpoint32_status='IN_PROGRESS',utc=datetime.now(timezone.utc).isoformat(),run=gate.directory.name,scope=result['scope'],coverage=coverage,regions=result['regions'],pending=result['pending'],emulation=emulation,negative_controls=dict(result='PASS',tests=10,log_sha256=sha(test.stdout.encode())),link_metadata_sha256=sha((gate.directory/'link.json').read_bytes()),artifacts=gate.link['artifacts'],verification_inputs=inputs,limitations=['One incremental build; no redundant second build at this intermediate batch','Every region listed in pending is unmatched and excluded','ABI exercise follows normal construction/model polling and virtual deletion; actual exception unwinding is not exercised','Original game engine and CRT implementations are outside source coverage','Source coverage is the union of independently certified regions, not a replacement DLL'])
