...
def draft(run, extra_seeds=None):
    ...
    def select(unit,symbol,rva):
        ...
        if unit in ('closure_models','closure_camera','closure_audio','closure_vehicle','closure_player','game_menu') and kind=='data' and symbol['name'].startswith('_') and not symbol['name'].startswith('__'):
