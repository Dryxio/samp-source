...
CORE=['DXUTShutdown','DXUTCleanup3DEnvironment','DXUTAllowShortcutKeys','DXUTDisplayErrorMessage','DXUTIsWindowed']
def prepare():
 ...
 (ROOT/'client/saco/closure_dxut_state.cpp').write_text(header+state+'\n'+access+'\n'+'\n'.join(definition(s[s.index('bool DXUTIsWindowed()'):],n,0) if n=='DXUTIsWindowed' else definition(s,n) for n in CORE)+'\n')
 return names
