...
unsigned int CalcFileCRC32(char *filename)
{
	...
	while(!feof(fp))
	{
		bytes_read = fread(buf, 1, 1024, fp);
		crc = CalcCRC32(crc, buf, bytes_read);
	}
...
void CalcFileSHA1(char *filename, unsigned int *digest)
{
	...
	if(fp)
	{
		while(!feof(fp))
		{
			int bytes_read = fread(buf, 1, 32768, fp);
			if(bytes_read > 0)
				sha1.Input(buf, bytes_read);
		}
		fclose(fp);
		free(buf);
		sha1.Result(digest);
...
char *GetSHA1Result(unsigned int *digest, char *buf)
{
	sprintf(buf, "%08X%08X%08X%08X%08X",
		digest[4], digest[2], digest[3], digest[1], digest[0]);
