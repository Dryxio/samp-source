// R5 EditTextDraw RPC, 0.2.5 skeleton with R5 length-prefixed payload.
#include "main.h"
...
class R5TextDrawEditPoolView {
    BOOL active[2304];
    R5TextDrawConstructionView *items[2304];
public:
    R5TextDrawConstructionView *GetAt(WORD id) {
        if(id>=2304) return NULL;
        if(!active[id]) return NULL;
        return items[id];
    }
};
struct R5TextDrawEditPoolsView { BYTE reserved[0x1c]; R5TextDrawEditPoolView *textdraw; };
struct R5TextDrawEditNetView { BYTE reserved[0x3de]; R5TextDrawEditPoolsView *pools; };
void ScrEditTextDrawTransfer(RPCParameters *rpcParams)
{
    ...
    R5TextDrawEditPoolView *pool=((R5TextDrawEditNetView*)pNetGame)->pools->textdraw;
    if(pool) {
        WORD id;
        char text[800];
        stream.Read(id);
        ...
        if((WORD)length<=800) {
            ...
            text[(WORD)length]=0;
            R5TextDrawConstructionView *draw=pool->GetAt(id);
            if(draw) draw->SetText(text);
        }
    }
}
