...
// Verified actual packed allocation/layout. No substitute classes.
typedef char R5HuffmanTreeSize[(sizeof(HuffmanEncodingTree)==1540)?1:-1];
typedef char R5HuffmanNodeSize[(sizeof(HuffmanEncodingTreeNode)==17)?1:-1];
typedef char R5HuffmanQueueSize[(sizeof(DataStructures::Queue<HuffmanEncodingTreeNode*>)==16)?1:-1];
typedef char R5HuffmanListSize[(sizeof(DataStructures::LinkedList<HuffmanEncodingTreeNode*>)==12)?1:-1];
