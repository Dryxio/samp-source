// Isolated command dependencies, verified R5 chat layout and pinned DXUT methods.
#include "main.h"
...
#pragma pack(1)
typedef struct _CHAT_PROCESS_EVAL_ENTRY
{
	DWORD timestamp;
	char prefix[28];
	char text[208];
	...
	DWORD textColor;
	DWORD prefixColor;
} CHAT_PROCESS_EVAL_ENTRY;

class CChatProcessEval // size: 25578
{
private:
	...
	char _gap12E[4];
	CHAT_PROCESS_EVAL_ENTRY	m_ChatWindowEntries[MAX_MESSAGES];
...
public:
 ...
 void ResetPage();
	...
	CChatProcessEval(IDirect3DDevice9 *pD3DDevice, CFontRender *pFontRender, CHAR *szChatLogFile);
};

void CChatProcessEval::ResetPage() { if(m_pScrollBar) m_pScrollBar->SetTrackPos(90); }
