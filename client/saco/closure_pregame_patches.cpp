// Direct existing dashr base source, R5 reconstruction candidate only.
// Never execute in this static matching session. Patch values are source-owned
// GTA destination data, not copied reference samp.dll instruction bytes.
#include "main.h"
#include <string.h>
extern int iGtaVersion;
extern DWORD dwSystemMemory;
extern DWORD dwStreamingMemory;
inline void RPCPreGameMakeWritable(DWORD addr,int size) {
 DWORD d;
...
BOOL ApplyPreGamePatches()
{
	...
	if(iGtaVersion == GTASA_VERSION_USA10) {
		RPCPreGameMakeWritable(ADDR_BYPASS_VIDS_USA10,6);
		*(BYTE *)ADDR_ENTRY = 5;
		memset((PVOID)ADDR_BYPASS_VIDS_USA10,0x90,6);
	}
	else if (iGtaVersion == GTASA_VERSION_EU10) {
		RPCPreGameMakeWritable(ADDR_BYPASS_VIDS_EU10,6);
		*(BYTE *)ADDR_ENTRY = 5;
		memset((PVOID)ADDR_BYPASS_VIDS_EU10,0x90,6);
	}

	// Loading screens
	RPCPreGameMakeWritable(0x866CD8,10);
	RPCPreGameMakeWritable(0x866CCC,10);
	strcpy((PCHAR)0x866CD8,"title");
	strcpy((PCHAR)0x866CCC,"title");

	RPCPreGameMakeWritable(0x745B87,68);
	memset((PVOID)0x745B87,0x90,68);

	RPCPreGameMakeWritable(0x7459E1,2);
	...
	*(PBYTE)0x561873 = 0xC0;
	...
	RPCPreGameMakeWritable(0x5B8E6A,4);
	*(DWORD *)0x5B8E6A = dwStreamingMemory;

	UnFuckAndCheck(0x4083C0,1,0xB8);
	*(PBYTE)0x4083C0 = 0xC3;

	RPCPreGameMakeWritable(0x590099,5);
	memset((LPVOID)0x590099,0x90,5);

	RPCPreGameMakeWritable(0x53E94C,1);
	*(PBYTE)0x53E94C = 2;

	RPCPreGameMakeWritable(0x731F60,4);
