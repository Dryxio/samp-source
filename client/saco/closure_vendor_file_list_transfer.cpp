...
{
	typedef char R5FileListReceiverSize[(sizeof(FileListReceiver)==1567)?1:-1];
...
typedef char R5FileTransferSize[(sizeof(FileListTransfer)==29)?1:-1];
typedef char R5FileTransferHuffmanSize[(sizeof(HuffmanEncodingTree)==1540)?1:-1];
typedef char R5FileTransferCallbackBaseSize[(sizeof(FileListTransferCBInterface)==4)?1:-1];
