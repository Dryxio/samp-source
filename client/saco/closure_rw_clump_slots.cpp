...
//B97A0/27, only first chunk stored; caller decides duplicate handling.
BOOL R5RwClumpDispatchView::SetStructure(R5RwChunk *chunk)
{
    if(!structure) {
        structure=chunk;
        return TRUE;
...
//B9740/27, only first chunk stored; caller decides duplicate handling.
BOOL R5RwClumpDispatchView::SetFrameList(R5RwChunk *chunk)
{
    if(!frameList) {
        frameList=chunk;
        return TRUE;
...
//B9760/27, only first chunk stored; caller decides duplicate handling.
BOOL R5RwClumpDispatchView::SetGeometryList(R5RwChunk *chunk)
{
    if(!geometryList) {
        geometryList=chunk;
        return TRUE;
...
//B9780/27, only first chunk stored; caller decides duplicate handling.
BOOL R5RwClumpDispatchView::SetAtomic(R5RwChunk *chunk)
{
    if(!atomic) {
        atomic=chunk;
        return TRUE;
...
//B97C0/27, only first chunk stored; caller decides duplicate handling.
BOOL R5RwClumpDispatchView::SetExtension(R5RwChunk *chunk)
{
    if(!extension) {
        extension=chunk;
        return TRUE;
