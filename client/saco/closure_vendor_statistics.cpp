...
// Reconstructed R5 sampling state. Names are descriptive, not original symbols.
extern unsigned int r5StatisticsLastTime;
extern uint64_t r5StatisticsLastSent;
extern uint64_t r5StatisticsLastReceived;
...
void StatisticsToString( RakNetStatisticsStruct *s, char *buffer, int verbosityLevel )
{
	...
	if ( verbosityLevel == 1 )
	{
		...
		double bpsSent;
		double bpsReceived;
		if (r5StatisticsLastTime == 0)
			r5StatisticsLastTime = s->connectionStartTime;
		elapsedTime = (int)(time-r5StatisticsLastTime) / 1000.0;
		bpsSent = (double)(unsigned int)(s->totalBitsSent-r5StatisticsLastSent) / elapsedTime;
		bpsReceived = (double)(unsigned int)(s->bitsReceived-r5StatisticsLastReceived) / elapsedTime;
		r5StatisticsLastSent = s->totalBitsSent;
		r5StatisticsLastReceived = s->bitsReceived;
		r5StatisticsLastTime = RakNet::GetTime();
		// Verbosity level 1
...
typedef char R5NetStatisticsSize[(sizeof(RakNetStatisticsStruct)==296)?1:-1];
